...
def build_and_extract_payload(src_path: str) -> bytes:
    ...
    obj_tmp_path = os.path.join(dirpath, f"{base_name[:-4]}.o")
    bin_tmp_path = obj_tmp_path[:-2]
    ...
    if execute_command(cmd) != 0 or not os.path.exists(obj_tmp_path):
        print_error("Compilation failed (nasm), object file was not created.")
        sys.exit(4)
    ...
    cmd.append('elf_i386')
    cmd.append('-o')
    cmd.append(bin_tmp_path)
    cmd.append(obj_tmp_path)

    # Check that the object file was created or abort.
    if execute_command(cmd) != 0 or not os.path.exists(obj_tmp_path):
        print_error("Compilation failed (ld), object file was not created.")
        sys.exit(4)
    ...
    print_success("\033[32;1mFiles in \n" + dirpath + "\n:\033[0m\n")
    return payload
